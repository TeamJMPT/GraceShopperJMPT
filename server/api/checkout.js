...
const { Cart, /*OrderHistory*/ } = require('../db/models')//Need order history model too
...
const checkout = router.post('/orderHistory', (req, res, next) => {
    const charge = stripe.charges.create(req.body, postStripeCharge(res))
    OrderHistory.create(charge)
    .then(newCharge => res.status(201).json(newCharge))
})
